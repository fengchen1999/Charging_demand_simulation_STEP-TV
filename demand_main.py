...
def read_travel_start_time():
    ...
    fig.savefig('./results/travel_start_time_distribution_NHTS2017.pdf', bbox_inches='tight', pad_inches=0, dpi=600)
...
def read_travel_start_SOC(step = 5):
    ...
    plt.legend()
    plt.show()
    fig.savefig("./results/init_SOC.pdf", bbox_inches='tight', pad_inches=0, dpi=600)
    ...
    plt.legend()
    plt.show()
    fig.savefig("./results/init_SOC_without_100.pdf", bbox_inches='tight', pad_inches=0, dpi=600)
...
def read_arrive_SOC():
    ...
    plt.show()
    fig.savefig("./results/arrive_SOC.pdf", bbox_inches='tight', pad_inches=0, dpi=600)
...
def fit_travel_start_SOC(initial_SOC_hist, step=5):
    ...
    plt.show()
    # plt.savefig("./results/init_SOC_fitting.pdf", bbox_inches='tight', pad_inches=0, dpi=600)
...
def fit_travel_start_time(travel_start_time_distribution):
    ...
    plt.show()
    fig.savefig('./results/travel_start_time_distribution_NHTS2017_fitting.pdf', bbox_inches='tight', pad_inches=0,
                dpi=600)
...
def gen_network():
    ...
    plt.show()
    fig.savefig("./results/Sioux_falls_road_network.pdf", bbox_inches='tight', pad_inches=0, dpi=600)
    return G
...
def fit_charging_pro():
    ...
    RMSEs = []
    popts = []
    # colors = ['#14517C', '#2F7FC1', '#E7EFFA', '#96C37D', '#F3D266', '#D8383A', '#F7E1ED', '#F8F3F9', '#C497B2', '#A9B8C6', '#F8AC8C', '#6F6F6F']
    colors = ['#1f77b4', '#ff7f0e', '#2ca02c', '#d62728', '#9467bd', '#8c564b', '#e377c2', '#7f7f7f', '#bcbd22', '#17becf', '#9edae5', '#ffbb78']
    ...
    # popt返回的是给定模型的最优参数。我们可以使用pcov的值检测拟合的质量，其对角线元素值代表着每个参数的方差。
        ...
        if i < 11:
            ax.scatter(x_expend, post_expend[i, :], color=colors[i],
                    label='t$\in$[' + str(i * 2) + ' ,' + str(i * 2 + 2) + ')') # plot posterior prob
        else:
            ax.scatter(x_expend, post_expend[i, :], color=colors[i],
                   label='t$\in$[' + str(i * 2) + ' ,' + str(i * 2 + 2) + ']')  # plot posterior prob
        if i == 5:
            ax.plot(x_plot, y_plot, label='Fitted curve', color=colors[i]) # plot fitting curve
        else:
            ax.plot(x_plot, y_plot, color=colors[i],)  # plot fitting curve
        MSE = np.power((f_y - post_expend), 2).sum() / (len(f_y))
    ...
    ax.legend(loc='upper right', ncol=2, fontsize=12, handlelength=1, labelspacing=0.4)
    # fig.set_size_inches(5, 6)
    plt.show()

    fig.savefig('./results/charging_pro_12curves_v2.pdf', bbox_inches='tight', pad_inches=0, dpi=600)
    print('1111')
...
# #
# with open("./results/init_fitting_results.pkl", "wb") as file:
...
with open("./results/charging_pro_fit_results_v2.pkl", "wb") as file:
    cloudpickle.dump([charging_pro_logit, MSEs, RMSEs, T_S_SOC_count, popts], file, True)

with open("./results/charging_pro_fit_results_v2.pkl", "rb") as file:
    tmp = cloudpickle.load(file)
    charging_pro_logit, MSEs, RMSEs, T_S_SOC_count, popts = tmp[0], tmp[1], tmp[2], tmp[3], tmp[4]
...
for i in range(len(trajs_multi_days)):
    for j in range(len(trajs_multi_days[i])):
        for k in range(len(trajs_multi_days[i][j].path_cut)):
            if trajs_multi_days[i][j].time_seq_cut[k] < 24:
                T_index = int(trajs_multi_days[i][j].time_seq_cut[k] // 1)
            else:
                T_index == 23
            S_index = trajs_multi_days[i][j].path_cut[k] - 1
            if trajs_multi_days[i][j].SOC_seq_cut[k] < 100:
            ...
            Days_T_S_demand[i, T_index, S_index] += \
                (100 - trajs_multi_days[i][j].SOC_seq_cut[-1]) * trajs_multi_days[i][j].battery_cap * 0.01
...
for i in range(12):
    for j in range(Days_T_SOC_prob.shape[0]):
        axs[i//4, i%4].scatter(x, Days_T_SOC_prob[j, i, :], color='b') # simulation data
    axs[i//4, i%4].scatter(x, arrive_SOC_matrix[i, :], color='r',
               label='[' + str(i * 2) + ' ,' + str(i * 2 + 2) + ']') # real data
    if i < 11:
...
plt.tight_layout()
plt.show()
fig.savefig('./results/charging_pro_simulation_v2.pdf', bbox_inches='tight', pad_inches=0, dpi=600)
...
fig.savefig('./results/traffic_flow_different_nodes.pdf', bbox_inches='tight'
...
plt.show()
fig.savefig('./results/traffic_flow.pdf', bbox_inches='tight'
            , pad_inches=0, dpi=600)
...
plt.tight_layout()
plt.show()
fig.savefig('./results/charging_demand_different_nodes.pdf', bbox_inches='tight'
            , pad_inches=0, dpi=600)
...
plt.tick_params(labelsize=12)
plt.show()
fig.savefig('./results/charging_demand.pdf', bbox_inches='tight'
            , pad_inches=0, dpi=600)
...
plt.legend()
plt.show()
fig.savefig('./results/charging_demand_hot_cold.pdf', bbox_inches='tight'
            , pad_inches=0, dpi=600)
...
def Curve_Fitting(x,y,deg):
    ...
    plt.show()
    fig.savefig('./results/relationship_temp_demand.pdf'
...
plt.legend()
plt.show()
fig.savefig('./results/traffic_flow_seasons.pdf', bbox_inches='tight', pad_inches=0, dpi=600)
...
plt.legend()
plt.show()
fig.savefig('./results/charging_demand_seasons.pdf', bbox_inches='tight', pad_inches=0, dpi=600)

# save data

with open("./results/data_to_robust_location.pkl", "wb") as file:
